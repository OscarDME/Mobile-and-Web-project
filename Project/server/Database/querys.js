export const querys = {
  //Usuarios
    ...
    //Cuestionario
    createCuestionario: "INSERT INTO Cuestionario (ID_UsuarioMovil, tiempo_disponible, ID_Objetivo, ID_NivelFormaFisica, ID_EspacioDisponible, ID_Musculo, Semanas ) OUTPUT INSERTED.ID_Cuestionario VALUES (@ID_UsuarioMovil, @tiempo_disponible, @ID_Objetivo, @ID_NivelFormaFisica, @ID_EspacioDisponible, @ID_Musculo, @Semanas)",
    ...
    // getEjercicios: "SELECT E.ID_Ejercicio, E.ejecucion, E.ejercicio, E.preparacion, E.ID_Modalidad, E.ID_Dificultad, D.dificultad AS Dificultad, M.modalidad AS Modalidad, Mu.descripcion AS Musculo, TE.descripcion AS Tipo_Ejercicio, EQ.equipo AS Equipo, L.lesion AS Lesion FROM Ejercicio E LEFT JOIN Dificultad D ON E.ID_Dificultad = D.ID_Dificultad LEFT JOIN Modalidad M ON E.ID_Modalidad = M.ID_Modalidad LEFT JOIN Musculo Mu ON E.ID_Musculo = Mu.ID_Musculo LEFT JOIN Tipo_Ejercicio TE ON E.ID_Tipo_Ejercicio = TE.ID_Tipo_Ejercicio LEFT JOIN Equipo EQ ON E.ID_Equipo = EQ.ID_Equipo LEFT JOIN Lesion L ON E.ID_Lesion = L.ID_Lesion WHERE E.estado = 1 AND E.ID_Modalidad != 4 ",
    getEjercicios: "SELECT TOP 100 E.ID_Ejercicio, E.ejecucion, E.ejercicio, E.preparacion, E.ID_Modalidad, E.ID_Dificultad, D.dificultad AS Dificultad, M.modalidad AS Modalidad, Mu.descripcion AS Musculo, TE.descripcion AS Tipo_Ejercicio, EQ.equipo AS Equipo, L.lesion AS Lesion FROM Ejercicio E JOIN Dificultad D ON E.ID_Dificultad = D.ID_Dificultad JOIN Modalidad M ON E.ID_Modalidad = M.ID_Modalidad LEFT JOIN Musculo Mu ON E.ID_Musculo = Mu.ID_Musculo LEFT JOIN Tipo_Ejercicio TE ON E.ID_Tipo_Ejercicio = TE.ID_Tipo_Ejercicio LEFT JOIN Equipo EQ ON E.ID_Equipo = EQ.ID_Equipo LEFT JOIN Lesion L ON E.ID_Lesion = L.ID_Lesion WHERE E.estado = 1 AND E.ID_Modalidad != 4 ",
    ...
    getAlimentoByID:"select a.ID_Alimento, a.nombre, a.calorias, a.peso, c.categoria from Alimento as a JOIN categoria as c ON a.ID_Categoria = c.ID_Categoria WHERE estado = 1 AND ID_Alimento = @ID_Alimento",
    ...
    getRecetaByID: "SELECT ID_Receta, receta, calorias, preparacion, link FROM Receta WHERE estado = 1 AND ID_Receta = @ID_Receta",
    ...
    getRutinasByUsuario: "SELECT R.ID_Rutina, R.nombre AS NombreRutina, U.nombre_usuario AS Autor, COUNT(DISTINCT DE.ID_Dia) AS CantidadDiasEntreno, R.duracion / 60 AS DuracionTotalMinutos FROM Rutina R INNER JOIN Usuario U ON R.ID_Usuario = U.ID_Usuario LEFT JOIN Dias_Entreno DE ON R.ID_Rutina = DE.ID_Rutina WHERE R.ID_Usuario = @ID_Usuario GROUP BY R.ID_Rutina, R.nombre, U.nombre_usuario, R.duracion",
    getRutinasPublicasByUsuario: "SELECT R.ID_Rutina, R.nombre AS NombreRutina, U.nombre_usuario AS Autor, COUNT(DISTINCT DE.ID_Dia) AS CantidadDiasEntreno, R.duracion / 60 AS DuracionTotalMinutos FROM Rutina R INNER JOIN Usuario U ON R.ID_Usuario = U.ID_Usuario LEFT JOIN Dias_Entreno DE ON R.ID_Rutina = DE.ID_Rutina WHERE R.publica = 1 AND R.ID_Usuario <> @ID_Usuario GROUP BY R.ID_Rutina, R.nombre, U.nombre_usuario, R.duracion",
...
SELECT 
  ...
  e.ID_Modalidad,
  e.ID_Musculo,
  DATEDIFF(SECOND, '00:00:00', CAST(ed.descanso AS TIME)) AS descanso,
  ed.superset,
  ed.ID_EjerciciosDia,
  m.descripcion AS Musculo
FROM 
  Ejercicio AS e
LEFT JOIN 
  EjerciciosDia AS ed ON e.ID_Ejercicio = ed.ID_Ejercicio
LEFT JOIN 
  Musculo AS m ON e.ID_Musculo = m.ID_Musculo
WHERE 
  ed.ID_Dias_Entreno = @ID_Dias_Entreno;
`,
    ...
    //Entrenamiento
    getWorkoutSessionDetails: "SELECT RSU.ID_ResultadoSeriesUsuario, RSU.ID_Serie, S.repeticiones, S.peso, CASE WHEN S.tiempo = '00:00:00' THEN NULL ELSE CONVERT(varchar, DATEADD(SECOND, DATEDIFF(SECOND, '00:00:00', S.tiempo), 0), 108) END AS tiempo, DATEDIFF(SECOND, '00:00:00', ED.descanso) AS descansoEnSegundos, S.ID_SeriePrincipal, E.ID_Ejercicio, E.ejercicio AS nombreEjercicio, E.ID_Musculo, D.dificultad AS Dificultad, ED.superset, M.descripcion AS Musculos, E.preparacion, E.ejecucion AS indicaciones, EQ.equipo AS material, TE.descripcion AS Tipo_Ejercicio, L.lesion AS Lesion, TM.modalidad AS Modalidad, CASE WHEN RA.ID_Usuario_Web IS NOT NULL AND RA.ID_UsuarioMovil IS NOT NULL THEN 'false' ELSE 'true' END AS Modificable FROM ResultadoSeriesUsuario RSU LEFT JOIN Serie S ON RSU.ID_Serie = S.ID_Serie LEFT JOIN ConjuntoSeries CS ON S.ID_Serie = CS.ID_Serie LEFT JOIN BloqueSets BS ON CS.ID_BloqueSets = BS.ID_BloqueSets LEFT JOIN EjerciciosDia ED ON BS.ID_EjerciciosDia = ED.ID_EjerciciosDia LEFT JOIN Dias_Entreno DE ON ED.ID_Dias_Entreno = DE.ID_Dias_Entreno LEFT JOIN Ejercicio E ON ED.ID_Ejercicio = E.ID_Ejercicio LEFT JOIN Dificultad D ON E.ID_Dificultad = D.ID_Dificultad LEFT JOIN Musculo M ON E.ID_Musculo = M.ID_Musculo LEFT JOIN Tipo_Ejercicio TE ON E.ID_Tipo_Ejercicio = TE.ID_Tipo_Ejercicio LEFT JOIN Equipo EQ ON E.ID_Equipo = EQ.ID_Equipo LEFT JOIN Lesion L ON E.ID_Lesion = L.ID_Lesion LEFT JOIN Modalidad TM ON E.ID_Modalidad = TM.ID_Modalidad LEFT JOIN Rutina_Asignada RA ON RSU.ID_Rutina_Asignada = RA.ID_Rutina_Asignada WHERE RSU.fecha = @fecha AND RSU.ID_Rutina_Asignada IN (SELECT ID_Rutina_Asignada FROM Rutina_Asignada WHERE ID_UsuarioMovil = @ID_UsuarioMovil AND fecha_eliminacion IS NULL)",
