                ...
                <div onClick={() => handleRowClick(exercise)} className={`row_header ${((selectedExercise && selectedExercise.id === exercise.id) || (editingExercise && editingExercise.id === exercise.id) || (addingExercise && addingExercise.id === exercise.id) || (eliminatingExercise && eliminatingExercise.id === exercise.id)) ? 'selected' : ''}`}>
                  ...
                  <div className="row_buttons">
                    <div className="row_edit">
                      <i className={`bi bi-database-add card-icon ${addingExercise && addingExercise.id === exercise.id ? 'selected' : ''}`} onClick={(e) => { e.stopPropagation(); handleAddClick(exercise); }}></i>
                    </div>
                    <div className="row_edit">
                        <i className={`bi bi-trash card-icon ${eliminatingExercise && eliminatingExercise.id === exercise.id ? 'selected' : ''}`} onClick={(e) => { e.stopPropagation(); handleDeleteClick(exercise); }}></i>
                    </div>
                    <div className="row_edit">
                      <i className={`bi bi-pencil-square card-icon ${editingExercise && editingExercise.id === exercise.id ? 'selected' : ''}`} onClick={(e) => { e.stopPropagation(); handleEditClick(exercise); }}></i>
                    </div>
                </div>
                </div>
