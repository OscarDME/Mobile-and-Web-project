...
const EditRoutineViewScreen = ({ navigation, route }) => {
  ...
  const handlePressAddExercise = (index) => {
    ...
    if (index < workouts.length && workouts[index].dayIndex !== undefined && workouts[index].ID_Dias_Entreno) {
      ...
      navigation.navigate('AnadirEjercicioView', {
        day: day,
        dayID: dayID,
  ...
  const renderDropdown = (index) => {
    ...
    return (
      <ModalDropdown
        ...
        disabled={true} // Deshabilita el dropdown
      />
    );
  };
  ...
  return (
    <View style={styles.container}>
      ...
      <TextInput
        ...
        editable={false} // Deshabilita la edición del input
      />
      <ScrollView style={styles.workoutsList}>
        {workouts.map((workout, index) => (
          <TouchableOpacity key={index} style={styles.workoutItem} onPress={() => handlePressAddExercise(index)}>
            {/* Elimina el TouchableOpacity para borrar día */}
        ...
        <View style={styles.switchContainer}>
          <Text style = {styles.publicatext}> {isPublic ? "Es rutina publica" : "No es rutina publica"}</Text>
        </View>
      </ScrollView>
    </View>
  );
...
export default EditRoutineViewScreen;
